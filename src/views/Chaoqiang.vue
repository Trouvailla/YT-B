<template>
  <div class="chaoqiang-page">
    ...
    <template v-else>
      ...
      <a-row :gutter="[16, 16]">
        <a-col :span="6" v-for="card in topCards" :key="card.label">
          <a-card :body-style="{ padding: '20px' }">
            ...
            <div class="kpi-value" style="color: #fa541c">{{ formatNum(card.value) }}</div>
          </a-card>
        </a-col>
      </a-row>
      ...
      <!-- 表格 -->
      <a-card style="margin-top: 16px" title="超抢手明细">
        <a-table
...
const barOpt = computed(() => {
  ...
  return {
    ...
    yAxis: { type: 'value' },
    series: [{ type: 'bar', data: s.map(x => x[1]), itemStyle: { color: '#fa541c' } }],
  }
})
